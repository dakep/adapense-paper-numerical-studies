...
fitted_vs_obs_plot <- ggplot(fitted_vs_obs_data, 
  ...
  scale_y_continuous(name = expression(paste('Fitted concentration of ', P[2], O[5], 
                                             ' [log % w/w]')),
                     breaks = seq(-4, 1, by = 1)) +
  scale_x_continuous(name = expression(
    paste('Obs. conc. of ', P[2], O[5], ' [log % w/w]')),
    breaks = seq(-4, 1, by = 1),
    minor_breaks = log(c(0.02, 0.06, 0.2, 0.6, 2, 6)),
    sec.axis = dup_axis(
      breaks = log(c(0.02, 0.06, 0.2, 0.6, 2, 6)),
      labels = c(0.02, 0.06, 0.2, 0.6, 2, 6),
      name = expression(paste('Obs. conc. of ', P[2], O[5], ' [% w/w]')))) +
...
pdf("../adapense-paper/figures/glass-combined.pdf", width = 7, height = 3)
grid.arrange(pred_perf_plot + labs(title = "(a) Prediction accuracy"),
             ...
             widths = grid::unit(c(0.68, 0.32), "npc"),
             heights = grid::unit(c(0.9, 0.1), "npc"))
dev.off()
```
