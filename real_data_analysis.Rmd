...
extract_test_pred_errors <- function (estimates, .id = NULL) {
...
cv_results <- list.files(CV_RESULTS_PATH, full.names = TRUE) %>% 
  map_dfr(function (fname) {
    ...
    tibble(seed = job_res$job$seed,
      ...
      cbind(extract_test_pred_errors(job_res$estimates))
  }) %>%
...
ggplot(fitted_vs_obs_data, aes(x = y, y = yfit, color = color, alpha = color)) +
...
Similarly for the CV estimates, the minima are well separated from the left boundary:
...
cv_curves %>% 
  ...
  mutate(lambda_rel = lambda / max(lambda),
         lambda_best = if_else(cvavg == min(cvavg), lambda/ max(lambda), NA_real_)) %>% 
  ungroup() %>% 
  ...
  geom_pointrange(aes(x = lambda_best), color = colorblind_cols('blue')) +
  scale_x_log10() +
