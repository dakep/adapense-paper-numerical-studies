##
## Simulation study -- Effect of good leverage points on variable selection
##
...
args <- arg_parser('Run the simulation study to uncover the effect of good leverage points.') %>%
  add_argument('--results-dir', type = 'character', help = 'Result path',
               default = file.path('results', 'simstudy', 'good_leverage')) %>%
...
SIM_LEV_POS <- 2^seq(-1.3, 14.3, by = 1.3)
...
#' Generate Simulated Data to Determine the Effect of Good Leverage Points
#'
#' @param lev_pos multiplicative factor for good and bad leverage points.
...
generate_data_good_lev <- function (lev_pos, test_n = 1000, test_seed = 7357) {
  settings <- list(
    n = 100,
    p = 32,
    s = 5,
    ...
    k_vert = -3,
    bad_lev_p = 0,
    bad_lev_p_relevant = 2,
    bad_lev_pos = lev_pos,
    good_lev_p = 5,
    good_lev_prop = 0.2,
    good_lev_pos = lev_pos,
    pve = 0.5,
    resid_dist = 'stable(alpha=1.33, beta=0)',
    pred_cor_type = 'ar1',
    pred_cor_strength = 2/3
  )
...
for (k_lev in SIM_LEV_POS) {
  job_id <- sprintf('s=%05d-klev=%.2f', args$job, k_lev)
  ...
  # Generate data
  set.seed(args$job)
  simdat <- generate_data_good_lev(k_lev)
