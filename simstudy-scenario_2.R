...
args <- arg_parser('Run the simulation study for scenario 2.') %>%
  add_argument('--results-dir', type = 'character', help = 'Result path',
               default = file.path('results', 'simstudy', 'scenario_02')) %>%
...
SIM_P <- 2^c(7, 5, 6) # number of predictors
SIM_RESID_DIST <- c('cauchy', 'stable(alpha = 1.33, beta = 0)', 'norm')  # error distributions
SIM_CONT_PROP <- c(0.25, 0) # contamination proportions
...
generate_data_scenario_2 <- function (p, resid_dist, contamination_percentage,
  ...
  settings <- list(
    ...
    k_vert = -3,
    bad_lev_p = floor(log2(p)),
    bad_lev_p_relevant = 2,
    bad_lev_pos = 2^8,
    good_lev_p = floor(log2(p)),
    good_lev_prop = 0.2,
    good_lev_pos = 4,
    pve = 0.5,
...
for (resid_dist in SIM_RESID_DIST) {
  for (cont in SIM_CONT_PROP) {
    for (p in SIM_P) {
      ...
      # Generate data
      set.seed(args$job)
      simdat <- generate_data_scenario_2(p, resid_dist, cont)
