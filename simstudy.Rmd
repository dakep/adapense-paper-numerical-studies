    ...
    if (is.list(x)) {
      if (is.null(x$test_pred_err)) {
        ...
        tibble(method = .id, 
               ...
               pred_err_rmspe = sqrt(mean(x$test_pred_err^2)) %||% NA_real_,
...
plot_pred_perf <- function (plot_d, ylim, guide, conf_level = 0.5, avg = median) {
  ...
  plot_obj <- plot_d %>% 
    mutate(pred_err = if_else(str_detect(resid_dist, 'Normal'),
                              pred_err_rmspe, pred_err_tau_size)) %>% 
...
plot_relative_pred_perf <- function (plot_d, ylim, guide, conf_level = 0.5, avg = median,
  ...
  plot_d <- plot_d %>% 
    ...
    mutate(pred_err_diff = pred_err / c(pred_err[method == baseline], NA_real_)[[1L]],
...
```{r scenario_1-prediction_accuracy-selected, fig.width=7, fig.height=3.75, out.width='100%'}
scenario_1 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM',
         !str_ends(method, fixed('EN')),
         !str_starts(resid_dist, fixed('Stable'))) %>% 
  plot_pred_perf(ylim = c(1, 1.2))
```

```{r scenario_1-prediction_accuracy, fig.width=7, fig.height=5.25, out.width='100%'}
scenario_1 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>% 
  plot_pred_perf(ylim = c(1, 1.2))
```

```{r scenario_1-prediction_accuracy-relative, fig.width=7, fig.height=5.25, out.width='100%'}
scenario_1 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>% 
...
```{r scenario_1-mcc, out.width='100%', fig.width=7, fig.height=3.75, }
scenario_1 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM',
         !str_starts(resid_dist, fixed('Stable'))) %>% 
...
```{r scenario_1-sens_spec, out.width='100%', fig.width=10, fig.height=5.25}
scenario_1 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>%
  {
...
```{r scenario_2-prediction_accuracy, fig.width=7, fig.height=5.25, out.width='100%'}
scenario_2 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>% 
  plot_pred_perf(ylim = c(1, 2))
```

```{r scenario_2-prediction_accuracy-relative, fig.width=7, fig.height=5.25, out.width='100%'}
scenario_2 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>% 
...
```{r scenario_2-mcc, out.width='100%', fig.width=7, fig.height=5.25}
scenario_2 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>% 
  plot_any_pct(mcc, ylab = 'MCC of variable selection', pct = FALSE, avg = median)
```

```{r scenario_2-sens_spec, out.width='100%', fig.width=10, fig.height=5.25}
scenario_2 %>% 
  filter(cv_type == 'se' | method == 'I-LAMM') %>%
  {
...
```{r good_leverage-sens_spec, out.width='100%', fig.width=7.1, fig.height=3.3}
good_leverage %>% 
  filter(cv_type == 'se' | method == 'I-LAMM', method != 'Ada. MM', method != 'MM') %>% 
